...
O ETL gera `dados/lab/` e `public/lab/datasets/` no ambiente de CI antes do build, de modo que o usuário não precisa commitar esses snapshots localmente. Veja [[Preparando Dados para o Lab]] para entender a decisão de manter `dados/lab/` em pasta visível e o que mudaria em versões futuras.
