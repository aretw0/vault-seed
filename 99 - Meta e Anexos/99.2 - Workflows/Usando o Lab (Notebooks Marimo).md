...
O ETL gera `.dgk/` e `public/lab/datasets/` no ambiente de CI antes do build — `.dgk/` é ignorada pelo git, então o usuário não precisa commitar esses snapshots. Veja [[Preparando Dados para o Lab]] para entender a separação entre `.dgk/` (runtime da CLI, efêmero) e `fontes/` (editável pelo usuário, commitado).
