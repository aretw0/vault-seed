# Compatibility shim — re-exports everything from the installable package.
# When dgk-lab-runtime is installed (pip install dgk-lab-runtime), this file
# delegates to it so notebooks get the versioned API without changes.
# When the package is not installed (e.g. marimo export without pip install),
# this file falls back to the inline implementation below so notebooks still work.
try:
    from dgk_lab_runtime import *  # noqa: F401, F403
    from dgk_lab_runtime import __version__  # noqa: F401
except ImportError:
    # Inline fallback — kept in sync with packages/lab-runtime/src/dgk_lab_runtime/__init__.py
    ...
    def _read_lab_json_runtime(candidates):
        ...
        for candidate in candidates:
            try:
                ...
                continue
        if last_error:
            raise RuntimeError("Não foi possível carregar o recurso de datasets.") from last_error
    ...
    def lab_altair_chart(chart):
        ...
        conforme o tema ativo (claro/escuro), então cores claras hard-coded sumiam
        no fundo escuro. Mantemos só a cor de marca padrão (série única), legível
        nos dois fundos, e o renderer SVG para o site poder ajustar o tema via CSS.
        """
    ...
    def with_data_provenance(payload, *, source: str, license: str = "verificar",
                             privacy: str = "private-until-published", collected_at: str = None):
        from datetime import datetime as _datetime, timezone as _timezone
        collected = collected_at or _datetime.now(_timezone.utc).isoformat().replace("+00:00", "Z")
        enriched = {
            "schemaVersion": 1, "source": source, "collectedAt": collected,
            "license": license, "privacy": privacy, "data": payload,
        }
    ...
    def parse_feed_xml(xml_text: str, *, source_url: str = None, limit: int = 50):
        ...
        if channel is not None:
            ...
            return {"schemaVersion": 1, "kind": "feed", "format": "rss",
    ...
    def fetch_local_url_text(url: str, *, timeout: int = 20, user_agent: str = "vault-seed-lab/1.0"):
        ...
        class _TextParser(_HTMLParser):
            def __init__(self):
                super().__init__()
                self._title = []; self._chunks = []; self._in_title = False; self._ignored = 0
        ...
        return {"url": url, "title": clean_lab_text(" ".join(parser._title)) or None,
                "text": text, "textPreview": text[:500],
