...
flowchart TD
    ...
    MainDecision -- "sensível ao tempo?" --> DailyDir
    MainDecision -- "tem prazo definido?" --> ProjectsDir
    MainDecision -- "responsabilidade contínua?" --> AreasDir
    MainDecision -- "conceito ou recurso?" --> KnowledgeDir
    MainDecision -- "projeto concluído?" --> ArchivesDir
