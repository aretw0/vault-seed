...
and returns disabled capabilities when absent. There is intentionally no route yet: the next T2 step must
ship an operator-useful wallet/issue/verify surface, not a proof-only page.
