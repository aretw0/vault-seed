...
**Goal:** Integrar Marimo ao vault-seed: dois notebooks do vault (publicação e grafo) exportados como WASM para `/<NOTEBOOKS_PATH>/` no GitHub Pages, mais dois starters para o usuário, com versão pinada via `requirements.txt` e Dependabot pip.

**Architecture:** O build Astro dispara uma integração customizada que escreve `public/<NOTEBOOKS_PATH>/vault-data.json` com metadados de todas as notas. `marimo export html-wasm` exporta os notebooks para `dist/<NOTEBOOKS_PATH>/`. O site GitHub Pages serve tudo de `dist/`. Notebooks vivem em `99 - Meta e Anexos/Notebooks/` e vão com o usuário.

**Configuração de rota:** O path onde os notebooks ficam publicados é controlado pela variável de ambiente `VAULT_NOTEBOOKS_PATH` (padrão: `lab`). Defini-la em `.env` ou no GitHub Actions substitui o padrão sem alterar código. O valor deve ser um segmento de URL sem barras (ex: `lab`, `notebooks`, `studio`). Todos os lugares que usam esse path — integração Astro, página de índice, workflow de deploy — devem ler do mesmo ponto de configuração.
...
| `.site/integrations/generate-vault-json.ts` | Criar | Astro integration: gera `public/$NOTEBOOKS_PATH/vault-data.json` |
| `astro.config.mjs` | Modificar | Adicionar `generateVaultJson()` + ler `VAULT_NOTEBOOKS_PATH` |
...
| `.site/pages/[notebooksPath]/index.astro` | Criar | Página Starlight de índice dos notebooks (rota dinâmica ou estática via config) |
...
| `.github/workflows/deploy-site.yml` | Modificar | Adicionar path triggers + steps de install + marimo export para `$VAULT_NOTEBOOKS_PATH` |

**Convenção de leitura do path em cada camada:**

```js
// astro.config.mjs e integrations/
...
```astro
---
// .site/pages/[lab]/index.astro (ou index.astro com path lido de import.meta.env)
const notebooksPath = import.meta.env.VAULT_NOTEBOOKS_PATH ?? 'lab';
---
```

O nome padrão `lab` foi escolhido por ser curto, não atrelar ao nome de nenhuma ferramenta, e sugerir espaço de exploração ativo — compatível com notebooks de análise, tutoriais interativos, experimentos ou qualquer outro uso que o usuário queira dar ao Marimo.
...
```bash
ls public/$NOTEBOOKS_PATH/vault-data.json
node -e "const d=require('./public/$NOTEBOOKS_PATH/vault-data.json'); console.log('notas:', d.notes.length, '| gerado:', d.generated)"
```
...
Nota: em WASM os starters buscam `"../vault-data.json"` (um nível acima de `starters/` no export). Para uso local, `os.getcwd()` aponta para a raiz do repo onde `public/$NOTEBOOKS_PATH/vault-data.json` existe.
...
**Files:**
- Create: `.site/pages/$NOTEBOOKS_PATH/index.astro`

- [ ] **Step 1: Criar `.site/pages/$NOTEBOOKS_PATH/index.astro`**
...
```bash
cat ".site/pages/$NOTEBOOKS_PATH/index.astro" | head -5
```
...
```bash
git add ".site/pages/$NOTEBOOKS_PATH/index.astro"
...
const fs = require('fs');
.load(fs.readFileSync('.github/dependabot.yml','utf8'));
console.log('OK');
" 2>/dev/null || python3 -c "
import yaml
.safe_load(open('.github/dependabot.yml').read())
print('OK')
" || python -c "
import yaml
.safe_load(open('.github/dependabot.yml').read())
      ...
      - name: Export analysis notebooks
        run: |
          ...
          marimo export html-wasm \
            ...
            --output dist/$NOTEBOOKS_PATH/publishing.html
          marimo export html-wasm \
            "99 - Meta e Anexos/Notebooks/analise-grafo.py" \
            --output dist/$NOTEBOOKS_PATH/graph.html
```

O `vault-data.json` já está em `dist/$NOTEBOOKS_PATH/` neste ponto — o Astro build copiou de `public/$NOTEBOOKS_PATH/` para `dist/$NOTEBOOKS_PATH/` automaticamente.
...
const yaml = require('js-yaml');
const fs = require('fs');
.load(fs.readFileSync('.github/workflows/deploy-site.yml','utf8'));
console.log('OK');
"
```
...
```bash
node -e "
const d = require('./public/$NOTEBOOKS_PATH/vault-data.json');
