...
## vault-seed development docs

Conventions and ongoing work for developing vault-seed itself live under `docs/` — in particular the
`docs/convergencia-*.md` family (consuming refarm as an SDK), whose `convergencia-refarm-status.md` is
the entry point. These are template-development docs, stripped by `initialize.yml` for end users.
