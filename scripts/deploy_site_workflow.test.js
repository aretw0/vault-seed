...
test("deploy-site workflow keeps GitHub Pages deploy gated by build and smoke", () => {
  ...
  assert.equal(
    ...
    "node scripts/clean_site_dist.mjs && pnpm --filter @aretw0/dgk-astro-plugins build && astro build",
  );
  ...
  assert.match(workflow, /run: pnpm --filter @aretw0\/dgk-astro-plugins build/);
  ...
  assert.ok(
    notebooksPathEnvCount >= 3,
    "build, notebook export, site check, and responsive smoke should honor VAULT_NOTEBOOKS_PATH",
  );
