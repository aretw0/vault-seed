...
const remarkCallouts = (options = {}) => {
    ...
    return (tree) => {
        visit(tree, 'blockquote', (node, index, parent) => {
            ...
            const openTag = `<aside class="callout callout-${normalizedType}" role="note" aria-label="${title}" data-callout="${rawType.toLowerCase()}"${colorStyle}>` +
            ...
            if (headerParagraphRemainder.length > 0) {
                bodyChildren.push({ type: 'paragraph', children: headerParagraphRemainder });
            }
...
//# sourceMappingURL=remark-callouts.js.map