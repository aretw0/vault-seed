...
describe("resolveContactsDir", () => {
  test("vault (padrão) aponta para dados/lab/contacts dentro do root", () => {
    const dir = resolveContactsDir("/meu/vault");
    assert.equal(dir, join("/meu/vault", "dados", "lab", "contacts"));
  });

  test("local aponta para ~/.dgk/contacts", () => {
  ...
  test("sem siloData usa vault como padrão", () => {
    const dir = resolveContactsDir("/root");
    assert.ok(dir.includes("dados/lab/contacts") || dir.includes("dados\\lab\\contacts"));
  });
});
