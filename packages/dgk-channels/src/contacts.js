 ...
 *   "vault"  — <vaultRoot>/dados/lab/contacts/<platform>.json  (git-tracked)
 *   "local"  — ~/.dgk/contacts/<platform>.json                  (machine-local)
 *   "/path"  — absolute path prefix                             (user manages sync)
 *
 * "vault" is the default and the right choice for private repos.
 * For public repos or orgs with sensitive topology, use "local" or a private path.
 */
...
export function resolveContactsDir(vaultRoot, siloData = {}) {
  const location = siloData?.contacts?.location ?? CONTACTS_LOCATION_VAULT;

  if (location === CONTACTS_LOCATION_VAULT) {
    return join(vaultRoot, "dados", "lab", "contacts");
  }
